...
public sealed class WhisperWindow : Window
{
    ...
    private VenuePlus.Services.MacroScheduler _scheduler;
    ...
    private VenuePlus.Configuration.ChatChannel _channel = VenuePlus.Configuration.ChatChannel.Whisper;
    ...
    private static string ChannelLabel(VenuePlus.Configuration.ChatChannel ch)
    {
        switch (ch)
        {
            case VenuePlus.Configuration.ChatChannel.Whisper: return "Whisper";
            case VenuePlus.Configuration.ChatChannel.Say: return "Say";
            case VenuePlus.Configuration.ChatChannel.Party: return "Party";
            case VenuePlus.Configuration.ChatChannel.Shout: return "Shout";
            case VenuePlus.Configuration.ChatChannel.Yell: return "Yell";
            case VenuePlus.Configuration.ChatChannel.FC: return "FC";
            case VenuePlus.Configuration.ChatChannel.Echo: return "Echo";
            case VenuePlus.Configuration.ChatChannel.Emote: return "Emote";
            default: return "Whisper";
        }
    }

    private static string BuildChannelCommand(VenuePlus.Configuration.ChatChannel ch, string text)
    {
        switch (ch)
        {
            case VenuePlus.Configuration.ChatChannel.Say: return "/say " + text;
            case VenuePlus.Configuration.ChatChannel.Party: return "/p " + text;
            case VenuePlus.Configuration.ChatChannel.Shout: return "/sh " + text;
            case VenuePlus.Configuration.ChatChannel.Yell: return "/y " + text;
            case VenuePlus.Configuration.ChatChannel.FC: return "/fc " + text;
            case VenuePlus.Configuration.ChatChannel.Echo: return "/echo " + text;
            case VenuePlus.Configuration.ChatChannel.Emote: return "/em " + text;
            default: return text;
        }
    }
    ...
    public WhisperWindow(VenuePlusApp app, ITargetManager targetManager, ICommandManager commandManager, IPluginLog log, VenuePlus.Services.MacroScheduler scheduler) : base("Macro Helper", ImGuiWindowFlags.AlwaysAutoResize)
    {
        _app = app;
    ...
    public void ExecuteSequenceFromText(string text)
    {
        ...
        var msgTrim = text?.Trim() ?? string.Empty;
        ...
        for (int i = 0; i < split.Length; i++)
        {
            ...
            {
                string final;
                if (_channel == VenuePlus.Configuration.ChatChannel.Whisper)
                {
                    if (t == null) continue;
        ...
        if (steps.Count > 0)
        {
            _scheduler.AddSteps(TranslateStepsToMacroSteps(steps));
            _status = "Sending...";
            _statusUntil = System.DateTimeOffset.UtcNow.AddSeconds(2);
        }
    }

    public void ExecuteSequenceFromText(string text, VenuePlus.Configuration.ChatChannel channel)
    {
        var prev = _channel;
        _channel = channel;
        ExecuteSequenceFromText(text);
        _channel = prev;
    }

    private static System.Collections.Generic.List<VenuePlus.Services.MacroStep> TranslateStepsToMacroSteps(System.Collections.Generic.List<SendStep> steps)
    {
        var list = new System.Collections.Generic.List<VenuePlus.Services.MacroStep>(steps.Count);
        for (int i = 0; i < steps.Count; i++)
        {
            list.Add(new VenuePlus.Services.MacroStep { Wait = steps[i].Wait, Seconds = steps[i].Seconds, Text = steps[i].Text });
        }
        return list;
    }
    ...
    public override void Draw()
    {
        ...
        {
            if (ImGui.Selectable("Whisper", _channel == VenuePlus.Configuration.ChatChannel.Whisper)) _channel = VenuePlus.Configuration.ChatChannel.Whisper;
            if (ImGui.Selectable("Say", _channel == VenuePlus.Configuration.ChatChannel.Say)) _channel = VenuePlus.Configuration.ChatChannel.Say;
            if (ImGui.Selectable("Party", _channel == VenuePlus.Configuration.ChatChannel.Party)) _channel = VenuePlus.Configuration.ChatChannel.Party;
            if (ImGui.Selectable("Shout", _channel == VenuePlus.Configuration.ChatChannel.Shout)) _channel = VenuePlus.Configuration.ChatChannel.Shout;
            if (ImGui.Selectable("Yell", _channel == VenuePlus.Configuration.ChatChannel.Yell)) _channel = VenuePlus.Configuration.ChatChannel.Yell;
            if (ImGui.Selectable("FC", _channel == VenuePlus.Configuration.ChatChannel.FC)) _channel = VenuePlus.Configuration.ChatChannel.FC;
            if (ImGui.Selectable("Echo", _channel == VenuePlus.Configuration.ChatChannel.Echo)) _channel = VenuePlus.Configuration.ChatChannel.Echo;
            if (ImGui.Selectable("Emote", _channel == VenuePlus.Configuration.ChatChannel.Emote)) _channel = VenuePlus.Configuration.ChatChannel.Emote;
            ImGui.EndCombo();
        }
        ImGui.Separator();
        ...
        var needsTarget = _channel == VenuePlus.Configuration.ChatChannel.Whisper;
        var canSend = split.Length > 0 && (hasAnyCmd || (hasAnyNonCmd && (!needsTarget || target != null)));
        ...
        var schedStatus = _scheduler.GetStatus();
        if (!string.IsNullOrEmpty(schedStatus))
        {
            _status = schedStatus;
            _statusUntil = System.DateTimeOffset.UtcNow.AddSeconds(2);
            if (string.Equals(schedStatus, "Sent", System.StringComparison.Ordinal) && !_persistMessage) _message = string.Empty;
        }
